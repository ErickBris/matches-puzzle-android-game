 ...
 * dropped. It is a subclass of MyAbsoluteLayout, which means that it is a
 * ViewGroup and views can be added as child views.
 * 
 * <p>
 * In the onDrop method, the view dropped is not added as a child view. Instead,
 * the view is repositioned within the DragLayer that contains the DropSpot. If
 * the DropSpot is not associated with a DragLayer, it will not accept dropped
 * objects.
 * 
 */
...
public class DropSpot extends ImageView implements DropTarget, DragController.DragListener {
	...
	// String xMatchesNum, yMatchesNum, rMatchesNum = "";

	DropSpot matchesImgs[] = new DropSpot[7];

	String[] mNumCodes = new String[] { "012456", "25", "02346", "02356", "1235", "01356", "013456", "025", "0123456", "012356" };
 ...
 */
	...
	private int mSavedBackground = Color.TRANSPARENT;
 ...
 */
	...
	public void onDrop(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo) {
		...
		act = (GameActivity) getContext();
		
		sou = new SoundClass(act);
		sou.playSound(R.raw.match);
		
		// The view being moved is stored in the dragInfo.
		DropSpot v = (DropSpot) dragInfo;

		operatorPlus = (DropSpot) act.mDragLayer.findViewById(R.id.operator0);

		// At some point, it would be nice to get away from use of the clone of
		...
		/*
		 * WindowManager.LayoutParams lp = mLayoutParams; lp.x = touchX -
		 * mRegistrationX; lp.y = touchY - mRegistrationY;
		 * mWindowManager.updateViewLayout(this, lp);
		 */

		// For now, take the view and update its position using the DragLayer
		// layout object.
		// The coordinates given to onDrop are relative to the DropSpot. Add in
		// its x and y location
		// so the drop occurs where the user expects it.
		// (At least the fact that the layout is absolute is pretty much hidden
		// from the caller.)
		...
		// this.setImageResource(R.drawable.stick2);

		if (this.getTag().equals("match_off")) {
			this.setImageResource(R.drawable.match_on);
			this.setTag("match_on");
		} else if (this.getTag().equals("match2_off")) {
			this.setImageResource(R.drawable.match2_on);
			this.setTag("match2_on");
		}

		if (v.getTag().equals("match_on")) {
			v.setImageResource(R.drawable.match_off);
			v.setTag("match_off");
		} else if (v.getTag().equals("match2_on")) {
			v.setImageResource(R.drawable.match2_off);
			v.setTag("match2_off");
		}

		this.setOnTouchListener(act);
		this.setup(null, act.dragController);

		v.setup(act.mDragLayer, act.dragController);
		v.setOnTouchListener(null);

		act.x = (getNumValue(act.xMatchesIDs) != 100) ? getNumValue(act.xMatchesIDs) : null;
		act.y = (getNumValue(act.yMatchesIDs) != 100) ? getNumValue(act.yMatchesIDs) : null;
		act.r = (getNumValue(act.rMatchesIDs) != 100) ? getNumValue(act.rMatchesIDs) : null;

		act.cMoves = act.cMoves + 1;
		
		act.movesInfo.setText(String.valueOf(act.cMoves) + " / " + String.valueOf(act.moves));

		if (act.cMoves == act.moves) {
			if (act.x != null && act.y != null && act.r != null) {
	...
	private int getNumValue(int[] matchesIDs) {
		...
		DropSpot matchesImgs[] = new DropSpot[7];
		for (int i = 0; i <= 6; i++) {
			matchesImgs[i] = (DropSpot) act.mDragLayer.findViewById(matchesIDs[i]);
			if (matchesImgs[i].getTag().equals(act.matchesTags[i])) {
	...
	public void onDragEnter(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo) {
		toast("onDragEnter");
		int bg = isEnabled() ? R.color.drop_target_enabled : Color.TRANSPARENT;
 ...
 */
	...
	/**
	 * Return true if this DropSpot is hooked up to a DragLayer. If it is, it
	 * means that it will accept dropped views.
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Set up the drop spot by connecting it to a drag layer and a drag
	 * controller. When this method completes, the drop spot is listed as one of
	 * the drop targets of the controller and the drop spot is also set as a
	 * DragListener.
	 * 
	 * If the dragLayer is null, the drop spot is set initially in a state where
	 ...
	 *            DragController
	 * @param initialColor
	 *            int - resource id of the initial background color
	 */
